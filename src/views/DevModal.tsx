...
const DevModal = ({ isOpen, onClose, onRefresh, dev }: AddDevModalProps) => {
	...
	return (
		<Modal
			...
			<ModalContent>
				{/* Header */}
				<ModalHeader>{dev ? 'Update dev' : 'Add dev'}</ModalHeader>
				{/* Close button */}
				...
				<ModalBody>
					<Flex gap={'0.5em'}>
						...
						<FormControl isDisabled={isLoading}>
							...
							<Input
								type='text'
								borderRadius={'1.5rem'}
								value={devForm.first_name}
								onChange={event => setDevForm({ ...devForm, first_name: event.target.value })}
							/>
						</FormControl>
						{/* Last name */}
						<FormControl isDisabled={isLoading}>
							...
							<Input
								type='text'
								borderRadius={'1.5rem'}
								value={devForm.last_name}
								onChange={event => setDevForm({ ...devForm, last_name: event.target.value })}
							/>
						</FormControl>
					</Flex>
					{/* Email */}
					<FormControl isDisabled={isLoading}>
						...
						<Input
							...
							borderRadius={'1.5rem'}
							value={devForm.email}
							onChange={event => setDevForm({ ...devForm, email: event.target.value })}
						/>
					</FormControl>
					{/* Age */}
					<FormControl isDisabled={isLoading}>
						...
						<Input
							...
							borderRadius={'1.5rem'}
							value={devForm.age || ''}
							onChange={event => setDevForm({ ...devForm, age: parseInt(event.target.value) })}
						/>
					</FormControl>
					{/* Country */}
					<FormControl isDisabled={isLoading}>
						...
						<Input
							type='textr'
							borderRadius={'1.5rem'}
							value={devForm.country}
							onChange={event => setDevForm({ ...devForm, country: event.target.value })}
						/>
					</FormControl>
				</ModalBody>
				{/* Footer */}
				<ModalFooter gap={'0.5em'}>
					<Button
						alignSelf={'end'}
						leftIcon={<Icon as={TbCheck} fontSize={'2xl'} />}
						colorScheme='blue'
						isLoading={isLoading}
						isDisabled={isLoading || !isFormValid}
						onClick={dev ? handleUpdateClick : handleSaveClick}>
						{dev ? 'Update' : 'Save'}
					</Button>
				</ModalFooter>
			</ModalContent>
